<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<?php if(!isset($respuestaCompleta)){?>
	<center>
	...
	<center>
		...
		<textarea name="cfdi" cols="100" rows="20">
			<?php
			if ($resultado=='ok'){
				printf($respuestaCompleta['xml']);
				// printf('<input type="text" name="nombre" value="'.$respuestaCompleta['uuid'].'" />');
			} else{
				printf($resultado);
			}
			?>
		...
		<?php
		if ($resultado=='ok'){
			printf('<b>UUID</b><br> <input type="text" size="38" name="nombre" value="'.$respuestaCompleta['uuid'].'" readonly=yes />');
